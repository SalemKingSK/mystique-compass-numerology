...
export const PSYCHOMATRIX_CELL_MEANINGS: Record<number, PsychomatrixCellData> = {
  ...
  7: {
    ...
    intro: '"The number 7 in the square of Pythagoras indicates nature\'s interest in revealing its talents. Nature protects him in all unexpected situations and accidents, which is interpreted as \'happiness\'. The bigger the \'7\', the bigger the task a person has to complete."',
    lineContext: 'Row 1 (1, 4, 7) — Purposefulness. Column 3 (7, 8, 9) — Talent potential. Ascending diagonal (3, 5, 7) — Temperament.',
    meanings: [
      { count: 0, label: 'No Special Talent or Luck from Nature', verbatim: 'Absence of Number 7 in Pythagoras Square means no special talent or luck from nature. The person must rely entirely on their own effort, energy, and learned skills.', scale: 'absent' },
      { count: 1, label: 'Very Weak Luck/Talent', verbatim: 'Very weak luck/talent. The protection of nature is barely perceptible. The person has a small task assigned by nature, and must work diligently to fulfill it.', scale: 'very-weak' },
      { count: 2, label: 'Luck/Talent Given in Norm', verbatim: 'Luck and talent given in norm. Nature provides a reasonable degree of protection and the person carries a meaningful task.', scale: 'norm' },
      { count: 3, label: 'Strong Luck, Nature Protects', verbatim: 'Strong luck. Nature protects the person in unexpected situations and accidents. The task assigned by nature is significant.', scale: 'special' },
      { count: 4, label: 'Sign of Alarm; Angels', verbatim: '"7777 is a sign of alarm. The angels — the ones with four sevens — descended to earth and die already in infancy. People with this sign should be very careful."', scale: 'strong' }
    ]
  },
  8: {
    digit: 8,
    cellName: 'Duty / Tolerance / Kindness',
    intro: '"The number 8 is one of the most important numbers that defines the entire Pythagorean square of a person. This number is responsible for a sense of duty to relatives (parents, family), tolerance and benevolence — qualities that we should show to parents and loved ones."',
    lineContext: 'Row 2 (2, 5, 8) — Family orientation. Column 3 (7, 8, 9) — Talent potential.',
    meanings: [
      { count: 0, label: 'No Sense of Duty to Relatives', scale: 'absent', verbatim: 'A person who does not have eights should not be relied on if he is entrusted with something — he is in no hurry to give back. There is no innate sense of duty to relatives.' },
      { count: 1, label: 'Very Weak Sense of Duty', scale: 'very-weak', verbatim: 'The presence of one eight in the psychomatrix begins to strongly influence the character of a person, because there is a possibility of transition of 11 to 8 or, vice versa, of 8 to 11.' },
      { count: 2, label: 'Duty/Tolerance Given in Norm; Self-Sacrifice', scale: 'norm', verbatim: 'For those capable of self-sacrifice. Two eights represent duty and tolerance given in normal measure.' },
      { count: 3, label: 'Strong Sense of Duty; Truth Finder', scale: 'special', verbatim: '"It is necessary to highlight the characteristic of duty for \'888\'. In this case we deal with the person who is kind, tolerant, remembers parents, but thus becomes the truth finder. This aim can be dangerous if it grows to maximum."' },
      { count: 4, label: 'Very Rare; Parapsychic Abilities', scale: 'strong', verbatim: 'A very rare sign. Such people often have parapsychic or extrasensory abilities.' }
    ]
  },
  9: {
    digit: 9,
...
export const PSYCHOMATRIX_TRANSITIONS: ComplementaryInsight[] = [
  ...
  {
    digits: [1, 1], // Sacrifice two 1s for an 8
  ...
  {
    digits: [5], // This is usually checked as a specific "absence" logic in UI
