...
export function buildCosmicProfile(
  ...
  const secondaryMissing = missingNumbers[1];

  const p2_opening = primaryMissing
    ? `The Lo Shu Grid speaks as clearly through its absences as its presences: ${MISSING_SHADOW[primaryMissing] || `the absent ${primaryMissing} marks a specific developmental frontier`}.`
    : `The Lo Shu Grid is remarkably complete — a rare configuration that distributes developmental challenges more evenly across the life.`;

  const p2_second_missing = secondaryMissing
    ? `A second absence compounds this: ${MISSING_SHADOW[secondaryMissing] || `the absent ${secondaryMissing} adds its own developmental weight`}.`
    : '';
  ...
  const p3_strength = primaryStrength
    ...
    : `The grid's configuration, even without a full arrow, shows a specific accumulation of strength in the numbers that are present.`;
  ...
  const p4_month = pmFocus
    ? `Within this, Personal Month ${currentPM} narrows the focus: ${pmFocus}, at this precise juncture in the year.`
    : '';

  const p4_year_relation = yearRelationLine
    ? `The broader environmental energy — as the ${chineseSign} encounters this year's animal — confirms: ${yearRelationLine}.`
    : '';
