...
export const PersonalYearChart: React.FC<PersonalYearChartProps> = ({
  ...
  const { cycleData, currentIndex, currentPYN, currentPower, currentYear } = useMemo(() => {
    // Use actual current date instead of hardcoded 2025/Jan 2026
    const now = new Date();
    const currentYear = now.getFullYear();

    // Personal Year Number (PYN) calculation: reduce(day + month + current_calendar_year)
    // This aligns the "Current" vertical line with the actual calendar year for a better user experience.
    const worldYear = reduce(currentYear);
    ...
    // 4 in the past, current year, 9 in the future = 14 years total
    for (let i = -4; i <= 9; i++) {
      const yearOffset = i;
    ...
    const currentIndex = cycleData.findIndex(d => d.year === currentYear);
    const currentPower = cycleData[currentIndex]?.power ?? 0;

    return { cycleData, currentIndex, currentPYN, currentPower, currentYear };
  ...
  useEffect(() => {
    ...
    setChartData({
      labels: cycleData.map(d => d.year.toString()),
      datasets: [
        {
