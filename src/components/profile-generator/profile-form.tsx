 ...
 *  • SVG constellation accent in the header area
 *  • Staggered mount animation for each field
 */
...
/* ── tiny helper: animated orbiting particle ring ── */
...
function ConstellationSVG() {
  return (
    ...
    >
      ...
      {/* Lines */}
      {[
        [20,60, 60,30], [60,30, 120,50], [120,50, 160,20],
        [160,20, 220,45], [220,45, 270,25], [270,25, 300,55],
      ].map(([x1,y1,x2,y2], i) => (
        <line key={i} x1={x1} y1={y1} x2={x2} y2={y2}
          stroke="#d4af3722" strokeWidth="0.8" />
      ))}
      {/* Stars */}
      {[
        [20,60,1.2],[60,30,2],[120,50,1.4],[160,20,2.5],
        [220,45,1.6],[270,25,2],[300,55,1.3],
      ].map(([cx,cy,r], i) => (
        <circle key={i} cx={cx} cy={cy} r={r} fill="#d4af37"
          filter="url(#pf-star-glow)" opacity="0.8">
    ...
    <>
      <style>{`
        ...
        .pf-btn:hover:not(:disabled) {
          ...
          box-shadow: 0 8px 48px rgba(212, 175, 55, 0.48);
        }
        ...
        .pf-card-title {
          ...
          font-size: 1.35rem;
          font-weight: 700;
          background: linear-gradient(135deg, #fdf1b8, #e8c56d, #d4af37);
        ...
        <div className="flex-grow flex items-center justify-center px-4 py-6">
          <div className="pf-card-wrap w-full">
            <div className="pf-card">
              ...
              >
                <div>
                  <div className="pf-card-title">Generate Profile</div>
                  <p className="text-purple-300/40 text-[0.6rem] font-cinzel tracking-widest mt-1 uppercase">
                    ✦ Cosmic Profile Generator ✦
                  </p>
                </div>
                <Button type="button" variant="ghost" size="icon"
                  onClick={onHistoryOpen}
                  className="text-amber-300/50 hover:text-amber-300 hover:bg-purple-900/30 rounded-full border border-amber-300/10 hover:border-amber-300/30 transition-all"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <motion.div custom={0} variants={fieldVariants} initial="hidden" animate="visible">
                  <Popover open={isSearchOpen} onOpenChange={setIsSearchOpen}>
                    ...
                    <PopoverContent
                      className="w-[var(--radix-popover-trigger-width)] p-0 bg-[#0f0523] border border-purple-500/20"
                      onOpenAutoFocus={e => e.preventDefault()}
                    >
                      <div className="max-h-80 overflow-y-auto">
                        {searchResults.length > 0 && (
                          <div className="border-b border-white/5 pb-1">
                            <div className="px-3 py-2 text-[9px] uppercase tracking-wider text-amber-300/40 font-bold bg-white/3 font-cinzel">
                              Local Database
                            </div>
                            {searchResults.map((person, i) => (
                              <div key={`${person.name}-${i}`} className="pf-search-result"
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={f.id}>
                        ...
                        <Input id={f.id} name={f.id} type="number"
                          placeholder={f.placeholder}
                          value={(formData as any)[f.id] || ''}
                          onChange={onFieldChange}
                          required min={f.min} max={f.max} />
                      </div>
                    ))}
                  </div>
                </motion.div>
                ...
                <motion.div custom={3} variants={fieldVariants} initial="hidden" animate="visible">
                  <label htmlFor="gender" className="pf-label">Gender</label>
                  <Select onValueChange={onSelectChange} value={formData.gender}>
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Select gender" />
                    </SelectTrigger>
                    <SelectContent className="bg-[#0f0523] border-purple-500/20">
                ...
                >
                  ...
                  <button type="submit" className="pf-btn" disabled={isPending}>
                    <div className="pf-btn-shimmer" />
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {isPending ? (
                        <><Loader2 className="h-4 w-4 animate-spin" /> Reading the cosmos…</>
                      ) : (
                        <><Sparkles className="h-4 w-4" /> Reveal My Cosmic Profile</>
                      )}
                    </span>
                  </button>
                </motion.div>
