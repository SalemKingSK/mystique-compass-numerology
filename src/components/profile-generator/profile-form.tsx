...
export function ProfileForm({
  ...
  React.useEffect(() => {
    if (searchQuery.trim().length > 1) {
      ...
      setSearchResults(famousBirthdays.filter(p => p.name.toLowerCase().includes(q) || (p.tags && p.tags.some(t => t.toLowerCase().includes(q)))).slice(0, 20));
  ...
  const fieldVariants = {
    ...
    visible: (i: number) => ({ opacity: 1, y: 0, transition: { delay: i * 0.07, duration: 0.45, ease: [0.23, 1, 0.32, 1] } }),
  };
