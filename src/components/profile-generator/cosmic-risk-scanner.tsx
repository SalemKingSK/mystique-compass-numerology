...
export function CosmicRiskScanner({ targetYear }: { targetYear: number }) {
  ...
  useEffect(() => { refreshMetas(); }, [targetYear, uniqueYears]);
  ...
  async function clearVault() {
    setDialog({
      message: `Delete all stored people data from Firestore for ${targetYear} conflict years?`,
      onConfirm: async () => {
        setDialog(null);
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs text-primary/80 font-cinzel">
                  ...
                  <button onClick={() => { abortRef.current = true; }}
                    className="text-rose-400 text-[9px] uppercase font-cinzel">Stop</button>
                </div>
            ...
            <Card className="glass-card p-6 border-primary/20">
              ...
              {scanning ? (
                ...
                <div className="text-center py-6 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-500 font-cinzel">Vault is empty. Run ingestion first.</p>
                </div>
              ) : (
            ...
            {scanResults.length > 0 && (
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <Input placeholder="Filter results..." value={filterQuery} onChange={e => setFilterQuery(e.target.value)}
                        ...
                        <Card key={`${person.wikidataId}-${idx}`} className="glass-card p-0 border-transparent overflow-hidden" style={{ borderLeft: `3px solid ${person.tier.color}` }}>
                          <button className="w-full p-4 flex items-start justify-between text-left gap-4" onClick={() => setExpandedId(expandedId === person.wikidataId ? null : person.wikidataId)}>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-primary/70 font-cinzel uppercase leading-relaxed mt-0.5 line-clamp-2">{person.description}</p>
                              ...
                              <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden bg-black/20 border-t border-white/5">
                                <div className="p-4 space-y-3">
                                  <p className="text-[11px] text-slate-300 leading-relaxed font-body italic">
                                    <span className="text-primary font-bold not-italic mr-1 uppercase font-cinzel">Headwind:</span>
                                    {person.name} ({person.animal}) faces a <span style={{ color: person.config.color }}>{person.config.label}</span> with the {targetYear} {targetSign.n} cycle.
                                  </p>
                                  <a href={person.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[9px] text-primary/70 hover:text-primary uppercase font-bold font-cinzel">
