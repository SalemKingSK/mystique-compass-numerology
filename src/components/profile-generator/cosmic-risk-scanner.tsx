...
export function CosmicRiskScanner({ targetYear }: { targetYear: number }) {
  ...
  useEffect(() => { refreshMetas(); }, [targetYear, refreshMetas]);
  ...
  async function ingestOneYear(year: number) {
    ...
    await setDoc(doc(db, META_COLL, String(year)), meta);
  ...
  async function startIngestion(yearsToIngest: number[]) {
    ...
    setIngesting(false);
    setIngestPhase(abortRef.current ? 'Stopped.' : 'Local Ingest requested ✓');
    await refreshMetas();
  }
  ...
  const handleCloudSync = useCallback(async () => {
    ...
    try {
      ...
      setCloudSyncError(`✓ Worker ran — ${d.monthsProcessed ?? 0} months processed.`);
      await refreshMetas();
    } catch (e: any) {
          ...
          <Card className="glass-card p-6 border-primary/20 space-y-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                Force Cloud Sync (Background Worker)
              </Button>

              {cloudSyncError && (
                <div className="flex items-start gap-2 p-3 bg-primary/10 border border-primary/30 rounded-xl">
                  <AlertTriangle className="h-3 w-3 text-primary/70 mt-0.5 shrink-0" />
                  <p className="text-[9px] font-cinzel text-primary/70 leading-relaxed break-all">
                    {cloudSyncError}
                  </p>
                </div>
              )}
            ...
            <div className="p-4 bg-primary/5 border border-primary/20 rounded-xl space-y-2">
              ...
              <p className="text-[11px] text-slate-400 font-body leading-relaxed">
                ...
                Use <strong className="text-slate-200">Force Cloud Sync</strong> to trigger the worker now.
              </p>
            </div>
              ...
              {scanning ? (
                ...
                <div className="text-center py-6 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-500 font-cinzel">
                    Vault is empty. Go to Data Vault and run sync first.
                  </p>
                </div>
              ) : (
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 bg-black/40 border-primary/20 font-body h-12"
                  />
                </div>
                              ...
                              >
                                <div className="p-4 space-y-3">
                                  <p className="text-[11px] text-slate-300 leading-relaxed font-body italic">
                                    {person.name} ({person.animal}) faces a <span style={{ color: person.config.color }}>{person.config.label}</span> with the {targetYear} cycle. 
                                    Combined with Personal Year {person.py}, this produces a composite risk score of <span style={{ color: person.tier.color }}>{person.totalScore}/6</span>.
                                  </p>
                                  <a
