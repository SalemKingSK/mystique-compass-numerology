...
// ═══════════════════════════════════════════════════════════════
// CHUNKS 11: PINNACLE & CHALLENGE DICTIONARIES (VERBATIM)
...
export function CosmicFateMap({ birthDay, birthMonth, birthYear }: CosmicFateMapProps) {
  ...
  useEffect(() => {
    ...
    if (!initialized.current) {
      ...
      (window as any).calculate(); // Auto-calculate on load
  ...
  const renderSynthesis = (py: number, uy: number, lp: number, bv: number, pm: number, pinnNum: number, challenge: number, pinnIdx: number, birthAnimalName: string, m: number, d: number, by: number, ry: number) => {
    ...
    const animalLine = ba && ya ? `Your ${birthAnimalName} nature meets a ${yearAnimalName} year (${catStr}) — ${cat === 'clash' ? 'an environment of maximum elemental friction calling for proactive adaptation rather than resistance' : cat === 'harm' ? 'a year of concealed pressures requiring extra vigilance in trust and documentation' : cat === 'destruction' ? 'a year when outdated structures may fracture, clearing ground for what genuinely serves you' : cat === 'ben-ming' ? 'your identity year, when all your characteristic patterns amplify to their fullest expression' : cat === 'alliance' ? 'an environmentally supported year where the collective field actively favours your initiatives' : 'a neutral year where outcomes reflect pure personal effort rather than exceptional external forces'}.` : '';
    ...
    const synthText = `In ${ry}, you are in a <strong>Personal Year ${py} — ${pyDesc}</strong>, riding the ${yr ? yr.phase.toLowerCase() : ''} phase of your nine-year cycle. The Universal Year ${uy} (${YD[uy] ? YD[uy].title : ''}) sets the collective backdrop — the shared frequency every person on earth is navigating alongside their personal arc. Your current Personal Month is <strong>${pm} (${pmNames[pm]})</strong>, offering a finer-grained window into this season's immediate texture. ${animalLine} ${convergeLine} Your active Pinnacle is <strong>${pinnNum}</strong> — the long-arc life theme operating beneath every annual cycle — while your active Challenge number <strong>${challenge === 0 ? '0 (The Master Test, requiring complete balance of all opposites)' : challenge + ' (' + lpName(challenge) + ')'}</strong> names the specific resistance pattern this life chapter asks you to develop through. Taken together, these layers describe not one story but several simultaneous ones: the year's momentum, the month's focus, the decade's theme, and the lifetime's direction — all converging in ${ry}.`;
  ...
  const renderZodiac = (m: number, d: number, by: number, currentYear: number, birthAnimalName: string) => {
    ...
    const chips = years.map(y => `
      <div class="zc" style="border-radius:10px;padding:11px 8px;text-align:center;cursor:pointer;transition:all 0.22s;position:relative;border:${y.danger === 4 ? '2px solid rgba(200,30,50,.85)' : y.danger === 3 ? '2px solid rgba(220,140,0,.8)' : `1px solid ${catColor(y.cat)}`}" onclick="window.openZodiacPop('${y.animalName}','${birthAnimalName}','${y.year}','${y.age}','${y.cat}')">
        ...
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.05rem;color:${['', '#e8b830', '#98b4de', '#68c268', '#c86040', '#dca030', '#de78a0', '#8870c8', '#a8b5cc', '#c84848'][y.py]};line-height:1;margin:3px 0">PY ${y.py}</div>
    ...
    document.getElementById('zodiac-container')!.innerHTML = `
      <div class="section-header">${ba.e} &nbsp; Your Chinese Zodiac — ${birthAnimalName} &nbsp; ${ba.e}</div>
      <p class="zodiac-intro">Born in a <strong style="color:var(--cf-gold)">${birthAnimalName}</strong> year — ${ba.el} element, ${ba.pol} polarity, Branch ${ba.br}. Your characteristic nature: <em>${ba.trait}</em>. Your health domains: ${ba.organ}. Your cardinal direction: ${ba.dir}. Below are your next 12 years mapped through Tai Sui astrology — click any year for detailed analysis.</p>
      <div class="zodiac-grid">${chips}</div>`;
  };
