...
/**
 * @fileOverview Complete stable implementation of Cosmic Fate Map.
 * Assembled verbatim from Chunks 01-11.
 */

export function CosmicFateMap() {
  useEffect(() => {
    // ═══════════════════════════════════════════════════════════════ // TTS ENGINE
    let ttsUtterance: SpeechSynthesisUtterance | null = null;
    let ttsActive: HTMLElement | null = null;

    function ttsPlay(btnEl: HTMLElement, text: string) {
      if (!window.speechSynthesis) return;
      if (ttsActive === btnEl && window.speechSynthesis.speaking) {
        window.speechSynthesis.cancel();
        btnEl.classList.remove('playing');
        btnEl.innerHTML = '<span class="tts-icon">🔊</span> Read Aloud';
        ttsActive = null;
        return;
      }
      if (window.speechSynthesis.speaking) window.speechSynthesis.cancel();
      if (ttsActive) {
        ttsActive.classList.remove('playing');
        ttsActive.innerHTML = '<span class="tts-icon">🔊</span> Read Aloud';
      }
      const clean = text.replace(/<[^>]+>/g, ' ').replace(/&amp;/g, 'and').replace(/&nbsp;/g, ' ')
      ...
      btnEl.innerHTML = '<span class="tts-icon">⏸</span> Stop';
      ttsActive = btnEl;
      window.speechSynthesis.speak(ttsUtterance);
    }

    function switchDash(btn: HTMLElement) {
    ...
    const ZOO: any = {
      ...
      Dragon: {
        ...
        allianceDesc: 'Rat and Monkey form your San He Water triangle — most auspicious for wealth, strategic intelligence, and large-scale achievement. Rooster years offer Liu He alliance — Dragon-Rooster pairing combines Dragon\'s visionary magnitude with Rooster\'s precision execution, exceptional for quality achievement and recognition. Dragon years are the most sought-after alliance years in Chinese astrology — you are the centerpiece of multiple triangular alliances, making your support enormously valuable to those who cultivate it.'
      },
      Snake: {
...
The 27/9 (Sceptre, wisdom recognized) promises that accumulated wisdom receives its fullest recognition and most effective expression this year. The 36/9 carries humanitarian achievement — personal completion opening naturally into collective service, individual journey becoming a gift to community simply through the authentic completeness of its telling.`,
    ...
    function findNextPY(month: number, day: number, targetPY: number, startYear: number) {
      for (let y = startYear; y <= startYear + 18; y++) {
        if (reduce(month + day + y) === targetPY) return y;
      }
      return startYear;
    }

    function getPersonalYearCalendar(month: number, day: number, birthYear: number, startYear: number, endYear: number) {
      const cal = [];
      for (let y = startYear; y <= endYear; y++) {
        const py = reduce(month + day + y);
        const uy = reduce(y);
        const animal = getAnimalFromYear(y);
        const birthAnimal = getAnimalFromBirthYear(birthYear);
        const cat = birthAnimal ? getCategory(Object.keys(ZOO).find(k => ZOO[k] === birthAnimal), Object.keys(ZOO).find(k => ZOO[k] === animal)) : 'neutral';
        cal.push({ year: y, py, uy, animalName: animal ? Object.keys(ZOO).find(k => ZOO[k] === animal) : '', cat });
      }
      return cal;
    }

    function buildPersonalIntersections(month: number, day: number, birthYear: number, readYear: number) {
      const cal = getPersonalYearCalendar(month, day, birthYear, readYear, readYear + 18);
      const py4years = cal.filter(c => c.py === 4);
      const py7years = cal.filter(c => c.py === 7);
      const birthAnimalName = Object.keys(ZOO).find(k => ZOO[k] === getAnimalFromBirthYear(birthYear)) || '';
      ...
      if (py4years.length > 0 || py7years.length > 0) {
        html += `<div class="section-header">🔥 &nbsp; Your Personal Critical Year Intersections &nbsp; 🔥</div><p style="text-align:center;font-size:.92rem;color:var(--cf-text-dim);line-height:1.8;max-width:680px;margin:0 auto 24px;font-style:italic">These are the specific years — calculated from your exact birth date — when Personal Years 4 and 7 intersect with your Chinese zodiac cycle. Each intersection has a unique character determined by the Tai Sui energy of that year.</p>`;
        if (py4years.length > 0) {
          html += `<div style="margin-bottom:8px"><div class="section-header" style="margin:20px 0 16px;font-size:.6rem">PY 4 — Foundation Year Intersections</div>${py4years.map(y => buildIntersectionCard(y, 4)).join('')}</div>`;
        }
        if (py7years.length > 0) {
          html += `<div><div class="section-header" style="margin:20px 0 16px;font-size:.6rem">PY 7 — Mystic Year Intersections</div>${py7years.map(y => buildIntersectionCard(y, 7)).join('')}</div>`;
        }
      }
      return html;
    }

    function buildConvergenceCards() {
      const outerHTML = CONVERGENCE_CARDS.map(c => {
        ...
        return `<div class="conv-card"> <div class="conv-header"> <div class="conv-title">${c.title}</div> <div class="conv-sub">${c.sub}</div> </div> <div class="conv-body" id="conv-body-${c.year}"> ${intro} <div class="enemy-grid">${chips}</div> <div class="wbox">${c.warning}</div> <div class="hbox">To discover exactly when PY ${c.year} will intersect with your Chinese zodiac cycle — and what that specific combination means for you — enter your birth date above and cast your fate map.</div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('conv-body-${c.year}').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> </div>`;
      }).join('');
      const innerHTML = CONVERGENCE_CARDS.map(c => {
        const intro = c.intro.split('\n\n').map(p => `<p class="cp">${p.trim()}</p>`).join('');
        const chips = c.chips.map(ch => `<div class="enemy-chip"><div class="enemy-chip-title">${ch.t}</div><p>${ch.p}</p></div>`).join('');
        return `<div class="conv-card"> <div class="conv-header"> <div class="conv-title">${c.title}</div> <div class="conv-sub">${c.sub}</div> </div> <div class="conv-body" id="conv-body-${c.year}-inner"> ${intro} <div class="enemy-grid">${chips}</div> <div class="wbox">${c.warning}</div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('conv-body-${c.year}-inner').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> </div>`;
      }).join('');
      const outer = document.getElementById('convergence-cards'); if (outer) outer.innerHTML = outerHTML;
      const inner = document.getElementById('convergence-cards-inner'); if (inner) inner.innerHTML = innerHTML;
    }

    function calculate() {
      const mInput = document.getElementById('cf-bMonth') as HTMLSelectElement;
      const dInput = document.getElementById('cf-bDay') as HTMLInputElement;
      const byInput = document.getElementById('cf-bYear') as HTMLInputElement;
      ...
      if (!m || !d) { alert('Please select your birth month and day.'); return; }
      if (!by || isNaN(by) || by < 1920 || by > 2006) { alert('Please enter your birth year (1920–2006).'); return; }
      ...
      let currentPinnacle, currentPinnacleNum;
      ...
      if (coreStrip) {
        coreStrip.innerHTML = `<div class="core-chip hl-py y${py}"> <div class="core-chip-label">Personal Year ${ry}</div> <div class="core-chip-num">${py}</div> <div class="core-chip-name">${YD[py] ? YD[py].title : ''}</div> </div> <div class="core-chip"> <div class="core-chip-label">Personal Month</div> <div class="core-chip-num" style="color:#de78a0">${pm}</div> <div class="core-chip-name" style="color:var(--cf-text-dim)">${pmNames[pm]}</div> </div> <div class="core-chip"> <div class="core-chip-label">Universal Year</div> <div class="core-chip-num" style="color:var(--cf-amethyst)">${uy}</div> <div class="core-chip-name">${YD[uy] ? YD[uy].title : ''}</div> </div> <div class="core-chip"> <div class="core-chip-label">Life Path</div> <div class="core-chip-num" style="color:var(--cf-jade-bright)">${lp}</div> <div class="core-chip-name">${lpName(lp)}</div> </div> <div class="core-chip"> <div class="core-chip-label">Birth Vibration</div> <div class="core-chip-num" style="color:var(--cf-silver)">${bv}</div> <div class="core-chip-name" style="color:var(--cf-text-dim)">${lpName(bv)}</div> </div> ${ba ? `<div class="core-chip"> <div class="core-chip-label">Chinese Sign</div> <div class="core-chip-num" style="color:var(--cf-gold-bright);font-size:2.2rem">${ba.e}</div> <div class="core-chip-name">${birthAnimalName}</div> </div>` : ''} <div class="core-chip"> <div class="core-chip-label">Pinnacle ${currentPinnacle} (Active)</div> <div class="core-chip-num" style="color:#68c268">${currentPinnacleNum}</div> <div class="core-chip-name" style="color:var(--cf-text-dim)">${lpName(currentPinnacleNum)}</div> </div> <div class="core-chip"> <div class="core-chip-label">Challenge ${currentPinnacle} (Active)</div> <div class="core-chip-num" style="color:#c86040">${currentChallenge}</div> <div class="core-chip-name" style="color:var(--cf-text-dim)">${currentChallenge === 0 ? 'The Master Test' : lpName(currentChallenge)}</div> </div>`;
      }
      
      const ab = document.getElementById('alert-banner');
      if (ab) {
        let alertText = '';
        if (py === uy) alertText = `<strong>⚡ Double Amplification:</strong> Personal Year ${py} aligns with Universal Year ${uy} — both vibrating at the same frequency. Expect intensified themes, accelerated timelines, and more dramatic experiences than either energy would produce alone.`;
        else if (py === bv) alertText = `<strong>✦ Core Identity Activation:</strong> Personal Year ${py} matches your Birth Vibration ${bv} — your deepest nature and current developmental phase in perfect alignment. Unusual clarity about who you are and where you're heading.`;
        else if (py === lp) alertText = `<strong>✦ Life Path Activation:</strong> Personal Year ${py} matches your Life Path ${lp} — a once-every-nine-years alignment of soul purpose and current cycle. Exceptional year for life-path-aligned decisions.`;
        else if (py === currentPinnacleNum) alertText = `<strong>◈ Pinnacle Resonance:</strong> Personal Year ${py} matches your active Pinnacle ${currentPinnacle} — the long-range life theme and the current annual vibration are in exact alignment. Major milestone year.`;
        if ((py === 4 && (lp === 5 || lp === 3)) || (py === 7 && (lp === 1 || lp === 6))) {
          const extra = `<strong>⚠ Life Path × Personal Year Tension:</strong> Your Life Path ${lp} creates significant dynamic friction with Personal Year ${py}. See your Critical Year Intersections and Convergence section below.`;
      ...
      const intersContainer = document.getElementById('personal-intersections-container');
      if (intersContainer) intersContainer.innerHTML = buildPersonalIntersections(m, d, by, ry);
      renderZodiac(m, d, by, ry, birthAnimalName);
      renderPinnacles(p1, p2, p3, p4, c1, c2, c3, c4, p1end, p2end, p3end, currentAge, lp);
      const resultArea = document.getElementById('result-area');
      if (resultArea) resultArea.classList.remove('result-hidden');
      const oracleBtn = document.querySelector('.dash-tab[data-panel="synthesis"]') as HTMLElement;
      if (oracleBtn) switchDash(oracleBtn);
      setTimeout(() => { document.getElementById('dash-nav')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
    }

    function renderYearDive(py: number, m: number, d: number, by: number, ry: number, birthAnimalName: string) {
      ...
      const yearAnimalData = ZOO[currentYearAnimal];
      let zodiacOverlay = '';
      if (ba && yearAnimalData) {
        ...
        else zodiacOverlay += `This is a Neutral year for ${birthAnimalName} — no special Tai Sui relationship creates heightened risk or support. Your Year ${py} numerological work proceeds primarily through personal effort rather than external energetic amplification.`;
        zodiacOverlay += '</div>';
      }
      ...
      const container = document.getElementById('year-dive-container');
      if (container) {
        container.innerHTML = `<div class="section-header">✦ &nbsp; Your Year ${py} — Deep Dive &nbsp; ✦</div> <div class="year-deep-dive y${py}${yr.isCrit ? ' critical' : ''}"> <div class="year-dive-header"> <div class="year-num-big">${py}</div> <div class="year-dive-title">${yr.title}</div> <div class="year-dive-sub">${yr.sub}</div> <div class="year-meta-row"> <span class="year-tag prim">${yr.planet}</span> <span class="year-tag" style="color:${yr.season === 'spring' ? '#6dba5a' : yr.season === 'summer' ? '#d4882a' : '#c06030'};border-color:${yr.season === 'spring' ? 'rgba(109,186,90,.4)' : yr.season === 'summer' ? 'rgba(212,136,42,.4)' : 'rgba(192,96,48,.4)'};background:${yr.season === 'spring' ? 'rgba(109,186,90,.08)' : yr.season === 'summer' ? 'rgba(212,136,42,.08)' : 'rgba(192,96,48,.08)'}">${yr.phase}</span> <span class="year-tag" style="color:var(--cf-silver-dim);border-color:rgba(168,184,208,.2);background:rgba(168,184,208,.04)">${yr.chakra}</span> ${ba && yearAnimalData ? `<span class="year-tag" style="color:${catColor(cat)};border-color:${catColor(cat)};background:rgba(0,0,0,.2)">${yearAnimalData.e} ${catLabel(cat)}</span>` : ''} ${yr.isCrit ? `<span class="year-tag" style="color:var(--cf-rose);border-color:var(--cf-rose-dim);background:rgba(192,58,90,.1)">⚠ Critical Year</span>` : ''} </div> </div> <div class="kw-strip">${kws}</div> <div class="tab-nav"> <button class="tab-btn active" onclick="window.swT('ov',this)">Overview</button> <button class="tab-btn" onclick="window.swT('py',this)">Pythagorean</button> <button class="tab-btn" onclick="window.swT('ve',this)">Vedic</button> <button class="tab-btn" onclick="window.swT('ch',this)">Chinese</button> <button class="tab-btn" onclick="window.swT('ca',this)">Chaldean</button> <button class="tab-btn" onclick="window.swT('pr',this)">Practices</button> </div> <div class="tab-content"> <div class="tab-panel active" id="tp-ov"> <div class="content-section" id="tp-ov-text"> ${paras(yr.overview)} ${zodiacOverlay} </div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('tp-ov-text').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> <div class="tab-panel" id="tp-py"> <div class="content-section"> <div class="content-h">Challenges, Shadows & Spiritual Curriculum</div> <div id="tp-py-text">${paras(yr.pyth)}</div> </div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('tp-py-text').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> <div class="tab-panel" id="tp-ve"> <div class="content-section"> <div class="content-h">Vedic Analysis — ${yr.planet}</div> <div id="tp-ve-text">${paras(yr.vedic)}</div> </div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('tp-ve-text').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> <div class="tab-panel" id="tp-ch"> <div class="content-section"> <div class="content-h">Chinese Numerology — Lo Shu, I Ching & Flying Stars</div> <div id="tp-ch-text">${paras(yr.chinese)}</div> </div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('tp-ch-text').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> <div class="tab-panel" id="tp-ca"> <div class="content-section"> <div class="content-h">Chaldean System — Compound Numbers & Karmic Vibration</div> <div id="tp-ca-text">${paras(yr.chald)}</div> </div> <button class="tts-btn" onclick="window.ttsPlay(this, document.getElementById('tp-ca-text').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button> </div> <div class="tab-panel" id="tp-pr"> <div class="content-section"> <div class="content-h">Integration Practices for Year ${py}</div> <div class="practice-grid">${prs}</div> </div> </div> </div> </div>`;
      }
    }

    function swT(name: string, btn: HTMLElement) {
      document.querySelectorAll('.tab-panel').forEach(p => p.classList.remove('active'));
      document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
      const p = document.getElementById(`tp-${name}`); if (p) p.classList.add('active');
      btn.classList.add('active');
    }

    function renderZodiac(m: number, d: number, by: number, currentYear: number, birthAnimalName: string) {
      ...
      const chips = years.map(y => `<div class="zc" style="border-radius:10px;padding:11px 8px;text-align:center;cursor:pointer;transition:all .22s;position:relative;border:${y.danger === 4 ? '2px solid rgba(200,30,50,.85)' : y.danger === 3 ? '2px solid rgba(220,140,0,.8)' : `1px solid ${catColor(y.cat)}`}" onclick="window.openZodiacPop('${y.animalName}','${birthAnimalName}','${y.year}','${y.age}','${y.cat}')"> <div class="zc-ani">${y.animal ? y.animal.e : '✦'}</div> <div class="zc-yr">${y.year}</div> <div style="font-family:'Cinzel Decorative',serif;font-size:1.05rem;color:${['', '#e8b830', '#98b4de', '#68c268', '#c86040', '#dca030', '#de78a0', '#8870c8', '#a8b5cc', '#c84848'][y.py]};line-height:1;margin:3px 0">PY ${y.py}</div> <div class="zc-type" style="color:${catColor(y.cat)};font-size:.5rem;text-transform:uppercase">${catLabel(y.cat)}</div> <div class="zc-age" style="font-size:.72rem;color:var(--cf-text-dim)">Age ${y.age}</div> </div>`).join('');
      const container = document.getElementById('zodiac-container');
      if (container) container.innerHTML = `<div class="section-header">${ba.e} &nbsp; Your Chinese Zodiac — ${birthAnimalName} &nbsp; ${ba.e}</div><div class="zodiac-grid">${chips}</div>`;
    }

    function openZodiacPop(animalName: string, birthAnimalName: string, year: string, age: string, cat: string) {
      const ba = ZOO[birthAnimalName]; const ya = ZOO[animalName]; if (!ba || !ya) return;
      let specificDesc = (cat === 'ben-ming') ? ba.benDesc : (cat === 'clash') ? ba.clashDesc : (cat === 'harm') ? ba.harmDesc : (cat === 'destruction') ? ba.destDesc : (cat === 'alliance') ? ba.allianceDesc : `This ${year} ${animalName} year is a Neutral period for ${birthAnimalName}.`;
      showPop(ya.e, `${year}: ${animalName} Year`, `${birthAnimalName} × ${animalName} — ${catLabel(cat)}`, `<div class="ibox"><strong>${year} (Age ${age})</strong> — Tai Sui: <span style="color:${catColor(cat)}">${catLabel(cat)}</span></div>${specificDesc.split('\n\n').map((p: string) => `<p class="cp">${p}</p>`).join('')}`);
    }

    function renderSynthesis(py: number, uy: number, lp: number, bv: number, pm: number, pinnNum: number, challenge: number, pinnIdx: number, birthAnimalName: string, m: number, d: number, by: number, ry: number) {
      const yr = YD[py]; const yearAnimalName = Object.keys(ZOO).find(k => ZOO[k] === getAnimalFromYear(ry)) || '';
      const cat = birthAnimalName && yearAnimalName ? getCategory(birthAnimalName, yearAnimalName) : 'neutral';
      const synthText = `In ${ry}, you are in a Personal Year ${py} — ${yr ? yr.title : ''}, riding the ${yr ? yr.phase.toLowerCase() : ''} phase of your nine-year cycle. Your current Personal Month is ${pm}. Your active Pinnacle is ${pinnNum}, while your active Challenge is ${challenge}. Taken together, these layers describe the convergence of your annual momentum and soul direction in ${ry}.`;
      const container = document.getElementById('synthesis-container');
      if (container) container.innerHTML = `<div style="position:relative"><div class="section-header">✦ &nbsp; Your ${ry} Reading — Oracle Synthesis &nbsp; ✦</div><div id="synthesis-text" style="font-size:1rem;line-height:2;color:var(--cf-text)">${synthText}</div><button class="tts-btn" style="margin-top:16px" onclick="window.ttsPlay(this, document.getElementById('synthesis-text').textContent)"><span class="tts-icon">🔊</span> Read Aloud</button></div>`;
    }

    function renderPinnacles(p1: number, p2: number, p3: number, p4: number, c1: number, c2: number, c3: number, c4: number, p1end: number, p2end: number, p3end: number, currentAge: number, lp: number) {
      const stages = [{ n: 1, p: p1, c: c1, label: `Birth — Age ${p1end}` }, { n: 2, p: p2, c: c2, label: `Age ${p1end + 1} — ${p2end}` }, { n: 3, p: p3, c: c3, label: `Age ${p2end + 1} — ${p3end}` }, { n: 4, p: p4, c: c4, label: `Age ${p3end + 1}+` }];
      const active = stages.find((s, idx) => {
        if (idx === 0) return currentAge <= p1end;
        if (idx === 1) return currentAge <= p2end;
        if (idx === 2) return currentAge <= p3end;
        return true;
      }) || stages[3];
      const container = document.getElementById('pinnacles-container');
      if (container) container.innerHTML = `<div class="section-header">◈ &nbsp; Your Pinnacles & Challenges &nbsp; ◈</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:11px">${stages.map(s => `<div style="background:rgba(18,30,58,.95);border:1px solid ${s.n === active.n ? '#68c268' : 'rgba(200,168,75,.1)'};border-radius:10px;padding:15px"> <div style="font-size:.56rem;color:${s.n === active.n ? '#68c268' : 'var(--cf-silver-dim)'}">Pinnacle ${s.n} ${s.n === active.n ? '◈ Active' : ''}</div> <div style="display:flex;gap:18px"> <div><div style="font-size:1.7rem;color:#68c268">${s.p}</div><div style="font-size:.48rem">Pinnacle</div></div> <div><div style="font-size:1.7rem;color:#c86040">${s.c}</div><div style="font-size:.48rem">Challenge</div></div> <div style="flex:1"><p style="font-size:.82rem">${PINNACLE_DESC[s.p]}</p></div> </div> </div>`).join('')}</div>`;
    }

    function showPop(g: string, t: string, s: string, b: string) {
      ...
      if (pb) pb.innerHTML = b;
      if (overlay) overlay.classList.add('visible');
      document.body.style.overflow = 'hidden';
    }
    function closePop() { document.getElementById('overlay')?.classList.remove('visible'); document.body.style.overflow = ''; }
    function closeOverlay(e: any) { if (e.target === document.getElementById('overlay')) closePop(); }

    function getAnimalFromBirthYear(by: number) { return Object.values(ZOO).find((z: any) => z.i === ((by - 2020) % 12 + 12) % 12); }
    function getAnimalFromYear(y: number) { return Object.values(ZOO).find((z: any) => z.i === ((y - 2020) % 12 + 12) % 12); }
    ...
    (window as any).swT = swT;
    (window as any).openZodiacPop = openZodiacPop;
    (window as any).closePop = closePop;
    (window as any).closeOverlay = closeOverlay;

    buildConvergenceCards();
    ...
    <div className="cosmic-fate-root relative min-h-screen overflow-hidden rounded-2xl">
      ...
      <div className="cf-page">
        <div className="cf-hero">
          ...
          <p className="hero-sub">Personal Year Oracle &amp; Numerological Guide</p>
          <div className="divider-orn">☽ &nbsp; Pythagorean · Vedic · Chinese · Chaldean &nbsp; ☾</div>
        </div>

        <div className="calc-card">
          <div className="calc-title">✦ &nbsp; Cast Your Personal Year &nbsp; ✦</div>
          <div className="calc-inputs">
            ...
            <div className="input-group">
              ...
              <input type="number" id="cf-bDay" placeholder="e.g. 15" min="1" max="31" />
            </div>
            <div className="input-group">
              <label>Birth Year</label>
              <input type="number" id="cf-bYear" placeholder="e.g. 1990" min="1920" max="2006" />
            </div>
            <div className="input-group">
      ...
      <div className="cf-overlay" id="overlay" onClick={(e) => (window as any).closeOverlay(e)}>
